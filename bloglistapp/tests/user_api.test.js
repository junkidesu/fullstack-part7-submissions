...
beforeEach(async () => {
  ...
  const userObjects = initialUsers.map(
    (u) =>
      new User({
        username: u.username,
        name: u.name,
...
describe('addition of a user', () => {
  ...
  describe('fails with appropriate status code and message if', () => {
    test('username is invalid', async () => {
      ...
      let result = await api.post('/api/users').send(noUsername).expect(400)

      expect(result.body.error).toContain('username missing')

      result = await api.post('/api/users').send(shortUsername).expect(400)

      expect(result.body.error).toContain('username shorter than 3 characters')

      result = await api.post('/api/users').send(existingUsername).expect(400)
    ...
    test('password is invalid', async () => {
      ...
      let result = await api.post('/api/users').send(noPassword).expect(400)

      expect(result.body.error).toContain('password missing')

      result = await api.post('/api/users').send(shortPassword).expect(400)
